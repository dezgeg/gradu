...
\begin{itemize}
    ...
    \item{Inode: sisältö pääosin samanlainen kuin ext2:n inode. Suurimpana erona on että Sprite LFS:n inoden sijaintia levyllä ei voi laskea pelkän inodenumeron avulla.}
    \item{Inodekartta (\emph{inode map}): tietorakenne jonka avulla voidaan paikantaa inoden sijainti levyllä inodenumeron perusteella.}
    \item{Segmentin käyttöastetaulu (\emph{segment usage table}): ylläpitää tietoa kunkin segmentin vapaasta tilasta.}
    \item{Segmentin yhteenveto (\emph{segment summary}): TODO}
    \item{Hakemistojen muutosloki (\emph{directory change log}): käytetään kaatumistilanteesta palautumiseen.}
\end{itemize}
