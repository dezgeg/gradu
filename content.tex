...
\begin{tabular}{l | l | l | l | l | p{7cm}}
    ...
    5  & 5.226    & +5.198   & 1    & 2    & Lohkoryhmien kuvaajat: vapaiden lohkojen määrää vähennetään          \\ \hline % 1
    6  & 5.226    & +0.000   & 4    & 1    & Lohkoryhmän 0 inodetaulu: inoden \texttt{new} attribuutit päivitetään ...      \\ \hline % 2
    7  & 5.226    & +0.000   & 8192 & 17   & Tiedoston \texttt{new} datalohkot sekä yksi epäsuora lohko                               \\        % 3
\end{tabular}
\end{table}
...
sekä Linuxiin vuodesta 2007 lähtien kehityksen alla ollut Btrfs~\cite{Btrfs} johon tutustutaan tässä kappeleessa.

Btrfs:n tavoitteena on olla seuraava uuden sukupolven yleiskäyttöinen tiedostojärjestelmä jota voitaisiin käyttää kaikkialla matalatehoisista laitteista, kuten älypuhelimista palvelinsaleihin.
Verrattuna Linuxin aiempaan vakiintuneeseen \texttt{ext4}-tiedostojärjestmään Btrfs kilpailee pääasiassa uusilla ominaisuuksilla,
...
Lähes kaikki Btrfs-tiedostojärjestelmän metadata sijaitsee B-puihin pohjautuvissa tietorakenteissa.
Btrfs:n kaikkien B-puiden avaimien koko on kiinteä 17 tavun kokoinen tietorakenne \texttt{btrfs\_key} kun taas arvon koko voi muuten vaihdella,
mutta arvon on mahduttava yhden B-puun lehtisivun sisälle.

Suurin osa perinteisistä tiedostojärjestelmän tietorakenteista sijaitsee \emph{alitiedostojärjestelmäpuussa FIXME} (\emph{subvolume tree}),
josta löytyvät sekä inodet, hakemistoalkiot että viittaukset tiedoston dataan.
Kutakin tiedostojärjestelmän inodea vastaa yksi alkio tyyppiä \texttt{INODE\_ITEM} jossa \texttt{objectid} sisältä inoden numeron, \texttt{offset}-kenttää ei käytetä ja alkion arvo sisältää tyypilliset inoden attribuutit.
Jokaista hakemistoalkiota vastaa B-puun alkio tyyppiä \texttt{DIR\_ITEM} jossa \texttt{objectid} sisältää hakemistoinoden numeron, \texttt{offset}-kenttä sisältää tiedostonimestä lasketun hajautusarvon ja alkion arvo sisltää itse tiedostonimen ja inodenumeron.
Inoden dataan viitataan \emph{extenteillä}.. FIXME (\emph{extent}) joka on usein lohkopuuta tehokkaampi menetelmä.
