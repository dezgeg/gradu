...
Tiedostojärjestelmän lokialueelle kirjoitetaan seuraavanlaista dataa:
...
\begin{itemize}
    ...
    \item{Segmentin käyttöastetaulun lohkot (\emph{segment usage table}): ylläpitää tietoa kunkin segmentin vapaasta tilasta.}
...
% Huomioitavaa on että ext2:een verrattuna Sprite LFS:stä puuttuu lohkobittikartta ja inodebittikartta. Vapaiden levylohkojen ... TODO
...
\begin{enumerate}
    ...
    \item{Uuden tiedoston \texttt{newfile} inode kirjoitetaan lokiin.}
    \item{Hakemiston \texttt{testdir} datalohkosta kirjoitetaan uusi versio lokiin johon on lisätty hakemistoalkio \texttt{newfile}.}
    \item{Hakemiston \texttt{testdir} inodesta kirjoitetaan uusi versio lokiin jossa datalohkoksi on vaihtunut edellisessä kohdassa kirjoitettu lohko.}
    ...
    \item{Segmentin käyttöastetaulun lohkoista kirjoitetaan uudet versiot lokiin, koska kohdissa 4.--6. tehdyt päivitykset aiheuttivat kuolleita lohkoja aiempiin lokisegmentteihin.}
    \item{Segmentin yhteenveto kirjoitetaan lokiin.}
    \item{Uusi tarkistuspiste kirjoitetaan, joka viittaa kohdissa 6. ja 7. kirjoitettuun inodekarttaan ja segmentin käyttöastetauluun.}
\end{enumerate}
...
Mikäli kaatuminen tapahtuu ennen uuden tarkistuspisteen kirjoittamisen aloittamista eli kohtien 1.--9. välissä,
...
Eteenkiertopalautuksen aikana käydään kahdesti läpi kaikki viimeisimmän tarkistuspisteen jälkeen tehdyt lokikirjoitukset~\cite{SpriteThesis}.
...
joiden avulla lokin sisältössä voidaan liikkua eteen- ja taaksepäin sekä tunnistaa lokin loppupää.
